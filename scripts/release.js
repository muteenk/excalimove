 ...
 * - bun run release                                 -> publishes packages with "test" tag and "-[hash]" version suffix
 * - bun run release --tag=test                      -> same as above
 * - bun run release --tag=next                      -> publishes packages with "next" tag and version "-[hash]" suffix
 * - bun run release --tag=next --non-interactive    -> skips interactive prompts (runs on CI/CD), otherwise same as above
 * - bun run release --tag=latest --version=0.19.0   -> publishes packages with "latest" tag and version "0.19.0" & prepares changelog for the release
 *
    ...
    rl.question(
      ...
      (answer) => {
        ...
        if (answer.toLowerCase() === "y") {
          execSync(`git add -u`);
          execSync(
            `git commit -m "chore: release @excalimove/excalimove@${nextVersion} 🎉"`,
          );
        } else {
          console.warn(
...
const buildPackages = () => {
  ...
  for (const packageName of PACKAGES) {
    console.info(`Building "@excalimove/${packageName}"...`);
...
const publishPackages = (tag, version) => {
  for (const packageName of PACKAGES) {
    ...
    console.info(
      `Published "@excalimove/${packageName}@${tag}" with version "${version}"! 🎉`,
    );
  }
};
